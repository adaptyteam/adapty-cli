...
export type AddKeywordActionFlags = {
    'cpt-bid'?: string | undefined;
    'cpt-bid-type'?: string | undefined;
    'match-type'?: string | undefined;
    'negate'?: string | undefined;
    'no-negate'?: boolean | undefined;
    'pause-original'?: boolean | undefined;
    'skip-enable-duplicates'?: boolean | undefined;
    'target-ad-group'?: string[] | undefined;
};
...
function resolveNegate(flags: AddKeywordActionFlags, params: Record<string, unknown>): Record<string, unknown> {
    if (flags.negate !== undefined) {
        return { enabled: true, type: flags.negate };
    }

    if (flags['no-negate'] === true) {
...
function resolveCptBidValue(
    ...
    if (cptBidType === 'set_to') {
        return bid === undefined ? storedCptBidValue(params) : Number(bid);
    }
...
function isSearchTermRule(operateWith: unknown): boolean {
    if (operateWith === SEARCH_TERM) {
        return true;
    }

    if (operateWith === TARGETING_KEYWORD) {
        return false;
    }
...
export function buildAddKeywordParams(
    ...
    if (cptBidValue === undefined) {
        missing.push('cpt-bid');
    }

    if (missing.length > 0) {
        throw missingFlagsError(missing);
    }
