...
export function decideConfirmation(opts: { isTty: boolean | undefined; json: boolean; yes: boolean }): ConfirmDecision {
    if (opts.yes) {
        return 'proceed';
    }

    if (opts.json || !opts.isTty) {
        return 'refuse';
    }
...
export function renderPreview(preview: MutationPreview): string {
    ...
    if (preview.body !== undefined) {
        lines.push(JSON.stringify(preview.body, null, 2));
    }
...
export async function confirmMutation(command: Command, preview: MutationPreview, yes: boolean): Promise<void> {
    const decision = decideConfirmation({ isTty: process.stdin.isTTY, json: command.jsonEnabled(), yes });
    ...
    try {
        ...
        if (!/^y(es)?$/i.test(answer.trim())) {
            command.error('Cancelled, nothing was sent.', { exit: 1 });
        }
    } finally {
        reader.close();
    }
