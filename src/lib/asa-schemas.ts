...
export type AsaMoney = {
    amount: string;
    currency: string;
};

export type AsaMetricsDTO = {
    avg_cpm: string;
    avg_cpt: string;
    impressions: number;
    ipm: string;
...
export type AsaLocInvoiceDetails = {
...
export type AsaCampaignGroupDTO = {
    currency: string;
    internal_id: string;
    last_synced_at: string;
    org_id: number;
...
export type AsaNegativeKeywordDTO = {
...
export type AsaAutomationRunDTO = {
    [key: string]: unknown;
    id?: string;
};

export type AsaAutomationRunEnqueuedDTO = {
    automation_id: string;
    dry_run: boolean;
    run_id: null | string;
};

export type AsaProductPageSyncDTO = {
    accepted_at: string;
    message: string;
    org_targets: number;
    replayed: boolean;
...
export type AsaCompetitorTopAppDTO = {
    adamId: number;
    avgSov: string;
    countries: string[];
    iconUrl: null | string;
...
export type AsaCompetitorContestedTermDTO = {
    competitorCount: number;
    maxSov: string;
    term: string;
};

export type AsaCompetitorsSummaryTotalDTO = {
...
export type AsaCompetitorAppTermsDTO = {
    adamId: number;
...
export type AsaCampaignMutationDTO = {
    campaign: AsaCampaignMutationEntity | null;
    errors: AsaMutationError[];
};

export type AsaCampaignMutationEntity = {
    campaign_group_id: string;
    campaign_id: number;
    internal_id: string;
    loc_invoice_details: AsaLocInvoiceDetails | null;
...
export type AsaAdGroupMutationDTO = {
    ad_group: null | Record<string, unknown>;
    errors: AsaMutationError[];
};

export type AsaAdMutationDTO = {
    ad: AsaAdDTO | null;
    errors: AsaMutationError[];
};

export type AsaKeywordMutationDTO = {
    errors: AsaMutationError[];
    is_validation_failure: boolean;
    keywords: Record<string, unknown>[];
};

export type AsaNegativeKeywordMutationDTO = {
    errors: AsaMutationError[];
    is_validation_failure: boolean;
    negative_keywords: Record<string, unknown>[];
};

export type AsaAutomationMutationDTO = {
    automation: AsaAutomationDTO | null;
};

export type AsaBulkOperationStatus = 'failed' | 'partial' | 'pending' | 'running' | 'success';

export type AsaBulkOperationAcceptedDTO = {
    operation_id: string;
};

export type AsaBulkOperationCountsDTO = {
    applied: number;
    failed: number;
    pending: number;
    total: number;
...
export type AsaBulkOperationObjectDTO = {
    apple_error_code: null | string;
    apple_error_message: null | string;
    entity_kind: string;
...
export type AsaBulkOperationStateDTO = {
...
export type AsaBulkOperationListItemDTO = {
    app_id: string;
    created_at: string;
    error: null | string;
    finished_at: null | string;
...
export type AsaBulkOperationListDTO = {
...
export type AsaTemplateIssueDTO = {
    column: null | string;
    message: string;
    row: null | number;
    sheet: null | string;
...
export type AsaBulkConvertResultDTO = {
