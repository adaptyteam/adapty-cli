...
function renderArrayItems(items: unknown[], indent: string): string[] {
    ...
    for (const item of items) {
        ...
        if (Array.isArray(item)) {
            ...
            const [head, ...rest] = renderObject(item as Record<string, unknown>, childIndent);

            if (head === undefined) {
                continue;
            }

            lines.push(`${indent}- ${head.slice(childIndent.length)}`, ...rest);
        }
    }
...
export function printResponse(data: Record<string, unknown>, log: (msg: string) => void): void {
    for (const line of renderObject(data, '')) {
        log(line);
    }
}
