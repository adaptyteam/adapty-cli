 ...
 */
export function draftFlowError(error: unknown, app: string, audiences: null | {content_type?: string; flow_id?: string}[]): null | string {
  if (!(error instanceof ApiError) || !error.message.includes(DRAFT_FLOW_MARKER)) return null

  const flows = (audiences ?? []).filter((a) => a.content_type === 'flow' && typeof a.flow_id === 'string')
  const flowId = flows.length === 1 ? flows[0].flow_id! : '<FLOW_ID>'
