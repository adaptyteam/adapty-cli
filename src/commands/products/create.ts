...
export default class ProductsCreate extends Command {
    static override description = 'Create a product with vendor products per platform';
    ...
    async run(): Promise<ProductDTO> {
        ...
        if (flags['ios-product-id']) {
            body.ios_product_id = flags['ios-product-id'];
        }

        if (flags['android-product-id']) {
            body.android_product_id = flags['android-product-id'];
        }

        if (flags['android-base-plan-id']) {
            body.android_base_plan_id = flags['android-base-plan-id'];
        }

        if (flags['stripe-product-id']) {
            body.stripe_product_id = flags['stripe-product-id'];
        }

        if (flags['stripe-price-id']) {
            body.stripe_price_id = flags['stripe-price-id'];
        }

        if (flags['paddle-product-id']) {
            body.paddle_product_id = flags['paddle-product-id'];
        }

        if (flags['paddle-price-id']) {
            body.paddle_price_id = flags['paddle-price-id'];
        }
