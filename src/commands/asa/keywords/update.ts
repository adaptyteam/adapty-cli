...
export default class AsaKeywordsUpdate extends Command {
    ...
    static override description = 'Change bid, status, text or match type of keywords';
    ...
    async run(): Promise<AsaKeywordMutationDTO> {
        ...
        for (const id of keywordIds) {
            if (!isValidUuid(id)) {
                this.error(`Invalid keyword ID format: ${id}`, { exit: 2 });
            }
        }

        const change: Record<string, unknown> = {};

        if (flags.bid !== undefined) {
            change.bid_amount = money(flags.bid, flags.currency);
        }
        ...
        if (flags['match-type'] !== undefined) {
            change.match_type = flags['match-type'];
        }

        if (flags.text !== undefined) {
            change.text = flags.text;
        }
