...
export default class AsaCampaignsUpdate extends Command {
    ...
    async run(): Promise<AsaCampaignMutationDTO> {
        ...
        if (flags.country !== undefined) {
            body.countries_or_regions = flags.country;
        }
        ...
        if (flags['target-cpa'] !== undefined) {
            body.target_cpa = money(flags['target-cpa'], flags.currency);
        }

        if (flags['bidding-strategy'] !== undefined) {
            body.bidding_strategy = flags['bidding-strategy'];
        }
        ...
        try {
            ...
            if (details) {
                body.loc_invoice_details = details;
            }
        } catch (error) {
        ...
        if (result.campaign && !replayed) {
            this.log('Campaign updated!');
        }
