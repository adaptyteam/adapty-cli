...
async function parseAppId(input: string): Promise<string> {
    if (!isValidUuid(input)) {
        throw new Error('Invalid app ID format. Run `adapty asa apps list` to find your app ID.');
    }
...
export default class AsaCampaignsBulkList extends Command {
    static override description = 'This company\'s bulk operations, newest first; inspect one with bulk-status';
    ...
    async run(): Promise<AsaBulkOperationListDTO> {
        ...
        printList(result.items, this.log.bind(this), {
            count: result.total,
            page: flags.page,
            pages,
        });
