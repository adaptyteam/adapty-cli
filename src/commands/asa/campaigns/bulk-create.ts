...
export default class AsaCampaignsBulkCreate extends Command {
    ...
    private async convertTemplate(
        client: ApiClient,
        flags: { 'from-file'?: string | undefined; 'org-id'?: number | undefined },
    ): Promise<Record<string, unknown>> {
        if (flags['org-id'] === undefined) {
            this.error('--org-id is required with --from-file.', { exit: 2 });
        }
        ...
        if (result.errors.length > 0 || result.request === null) {
            for (const issue of result.errors) {
                this.log(this.formatIssue(issue));
            }
    ...
    private countStructure(body: Record<string, unknown>): StructureCounts {
        ...
        for (const campaign of campaigns) {
            ...
            for (const adGroup of adGroups) {
                if (typeof adGroup !== 'object' || adGroup === null) {
                    continue;
                }
    ...
    private report(state: AsaBulkOperationStateDTO): void {
        ...
        for (const object of failedObjects) {
            printResponse(object, this.log.bind(this));
            this.log('---');
        }

        if (state.status === 'failed') {
            this.error('Bulk operation failed.', { exit: 1 });
        }

        if (state.status === 'partial') {
            this.warn('Some objects were not created — see the failures above.');
        }
    }
