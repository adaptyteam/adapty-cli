...
export default class AsaNegativeKeywordsAdd extends Command {
    static override description = 'Add negative keywords to an ad group or across a campaign';
    ...
    async run(): Promise<AsaNegativeKeywordMutationDTO> {
        ...
        if (!target) {
            this.error('Pass either --ad-group or --campaign.', { exit: 2 });
        }

        if (!isValidUuid(target)) {
            this.error('Invalid ID format.', { exit: 2 });
        }
        ...
        // eslint-disable-next-line no-nested-ternary -- FIXME if you see this
        ...
        const { replayed, result } = await asaWrite<AsaNegativeKeywordMutationDTO>(client, 'post', '/negative-keywords', {
            body,
