...
const sentBody = (stub: sinon.SinonStub, callIndex: number): Record<string, unknown> =>
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-return -- FIXME if you see this
...
const sentParams = (stub: sinon.SinonStub, callIndex: number): Record<string, unknown> => {
    const { actions } = sentBody(stub, callIndex) as { actions: { params: Record<string, unknown> }[] };
    const [first] = actions;

    if (first === undefined) {
        throw new Error('expected the request to carry at least one action');
    }
...
describe('asa automations add-as-keyword action flags', () => {
    ...
    it('update reads the rule, rebuilds the action and sends the whole actions list', async () => {
        ...
        const body = sentBody(fetchStub, 1) as { actions: { params: unknown; type: string }[] };
        expect(body.actions).to.have.length(1);

        const [action] = body.actions;

        expect(action?.type).to.equal('add-as-keyword-to');
