...
export function assertFetch({ base = API_BASE, body, callIndex, headers, method, path, query, stub }: AssertFetchOpts): void {
    ...
    if (body) {
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion, @typescript-eslint/no-unsafe-assignment -- FIXME if you see this
        const actual = JSON.parse(init.body!);
