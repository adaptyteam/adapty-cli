...
describe('confirmation', () => {
    ...
    // process.stdin.isTTY is undefined, not false, on a pipe — @types/node declares it `boolean`
    it('refuses on a pipe, where isTTY is undefined rather than false', () => {
        expect(decideConfirmation({ isTty: undefined, json: false, yes: false })).to.equal('refuse');
        expect(decideConfirmation({ isTty: undefined, json: false, yes: true })).to.equal('proceed');
    });
